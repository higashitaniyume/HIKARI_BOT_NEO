...
class IsaacWikiError(RuntimeError):
    pass


class IsaacWikiNotFound(IsaacWikiError):
    pass


@dataclass(slots=True)
class IsaacWikiResult:
    title: str
    summary: str
    detail: str
    url: str
...
class IsaacWikiClient:
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.user_agent = format_bot_name_text(config.get("user_agent") or "{bot_name} isaac_wiki")
        if not self.api_url:
            raise IsaacWikiError("以撒 Wiki API 地址未配置")
    ...
    async def search(self, query: str) -> IsaacWikiResult:
        keyword = query.strip()
        if not keyword:
            raise IsaacWikiError("缺少搜索关键词")
    ...
    async def _request(self, params: dict[str, Any]) -> dict[str, Any]:
        try:
            ...
            raise IsaacWikiError(f"以撒 Wiki 连接失败: {type(e).__name__}") from e
        except httpx.HTTPStatusError as e:
            raise IsaacWikiError(f"以撒 Wiki 请求失败: HTTP {e.response.status_code}") from e
        except ValueError as e:
            raise IsaacWikiError("以撒 Wiki 返回内容不是有效 JSON") from e
        if not isinstance(data, dict):
            raise IsaacWikiError("以撒 Wiki 返回格式异常")
        return data
    ...
    async def _search_page(self, keyword: str) -> dict[str, Any]:
        ...
        if not isinstance(pages, list) or not pages:
            raise IsaacWikiNotFound(f"没有找到「{keyword}」")
        ...
        if not isinstance(page, dict) or not page.get("title") or not page.get("fullurl"):
            raise IsaacWikiError("以撒 Wiki 搜索结果格式异常")
        return page
