...
class McWikiNotFound(McWikiError):
    pass
...
class McWikiClient:
    ...
    async def search(self, query: str) -> McWikiResult:
        keyword = query.strip()
        if not keyword:
            raise McWikiError("缺少搜索关键词")
    ...
    async def _request(self, params: dict[str, Any]) -> dict[str, Any]:
        try:
            ...
            raise McWikiError(f"Minecraft Wiki 连接失败: {type(e).__name__}") from e
        except httpx.HTTPStatusError as e:
            raise McWikiError(f"Minecraft Wiki 请求失败: HTTP {e.response.status_code}") from e
        except ValueError as e:
            raise McWikiError("Minecraft Wiki 返回内容不是有效 JSON") from e
        if not isinstance(data, dict):
            raise McWikiError("Minecraft Wiki 返回格式异常")
        return data
    ...
    async def _search_page(self, keyword: str) -> dict[str, Any]:
        ...
        if not isinstance(pages, list) or not pages:
            raise McWikiNotFound(f"没有找到「{keyword}」")
        ...
        if not isinstance(page, dict) or not page.get("title") or not page.get("fullurl"):
            raise McWikiError("Minecraft Wiki 搜索结果格式异常")
        return page
