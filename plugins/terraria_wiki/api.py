...
class TerrariaWikiError(RuntimeError):
    pass


class TerrariaWikiNotFound(TerrariaWikiError):
    pass


@dataclass(slots=True)
class TerrariaWikiResult:
    title: str
    summary: str
    detail: str
    url: str
...
class TerrariaWikiClient:
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.user_agent = format_bot_name_text(config.get("user_agent") or "{bot_name} terraria_wiki")
        if not self.api_url:
            raise TerrariaWikiError("泰拉瑞亚 Wiki API 地址未配置")
    ...
    async def search(self, query: str) -> TerrariaWikiResult:
        keyword = query.strip()
        if not keyword:
            raise TerrariaWikiError("缺少搜索关键词")
    ...
    async def _request(self, params: dict[str, Any]) -> dict[str, Any]:
        try:
            ...
            raise TerrariaWikiError(f"泰拉瑞亚 Wiki 连接失败: {type(e).__name__}") from e
        except httpx.HTTPStatusError as e:
            raise TerrariaWikiError(f"泰拉瑞亚 Wiki 请求失败: HTTP {e.response.status_code}") from e
        except ValueError as e:
            raise TerrariaWikiError("泰拉瑞亚 Wiki 返回内容不是有效 JSON") from e
        if not isinstance(data, dict):
            raise TerrariaWikiError("泰拉瑞亚 Wiki 返回格式异常")
        return data
    ...
    async def _search_page(self, keyword: str) -> dict[str, Any]:
        ...
        if not isinstance(pages, list) or not pages:
            raise TerrariaWikiNotFound(f"没有找到「{keyword}」")
        ...
        if not isinstance(page, dict) or not page.get("title") or not page.get("fullurl"):
            raise TerrariaWikiError("泰拉瑞亚 Wiki 搜索结果格式异常")
        return page
