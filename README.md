...
> [!IMPORTANT]
> 机器人本体不读取 `.env`。机器人运行配置来自 `BotData/config.json` 和 `BotData/plugin_configs/*.json`；根目录 `.env` 只给 Docker Compose 设置端口、镜像名和 NapCat 账号。
...
| TTS 说话 | `说话 <文本>`、`永雏塔菲 <文本>` | 使用 Edge TTS 或 Fish Audio 合成语音并发送 |
...
- 贴纸素材转换：Docker 镜像内已包含 `ffmpeg`、Cairo/Pango 等转换依赖
...
适合普通使用者，不需要 clone 整个项目。先创建一个空目录，例如：

```bash
mkdir -p /opt/hikaribot-docker
cd /opt/hikaribot-docker
```

然后放入下面这个 `docker-compose.yml`：
...
脚本第一次生成配置后会退出。编辑 `BotData/config.json` 和 `BotData/plugin_configs/*.json`，再运行一次脚本即可构建并启动。

### 部署到服务器

仓库内的 `deploy.ps1` 是作者自用的一键远端部署脚本，默认部署到 `root@192.168.31.2:/opt/hikaribot-dockcer`。其他服务器可以改参数：
...
本插件支持 Edge TTS 和 Fish Audio 两种在线语音合成。`说话 <文本>` 使用配置里的 `default_provider`，默认是 Edge TTS；`永雏塔菲 <文本>` 固定使用 Fish Audio 的配置音色。生成的音频会放到 `/tmp/hikari_bot/tts`，再通过 OneBot 语音消息发送。
...
|------|------|
| `enabled` | 是否启用 `说话` 命令 |
| `default_provider` | `说话` 命令默认引擎，支持 `edge` 或 `fish_audio` |
| `voice` | Edge TTS 音色，默认 `zh-CN-XiaoxiaoNeural` |
| `rate` | 语速，例如 `+0%`、`-20%`、`+20%` |
| `volume` | 音量，例如 `+0%`、`-20%` |
| `pitch` | 音高，例如 `+0Hz`、`-20Hz` |
| `fish_audio.api_key` | Fish Audio API Key |
| `fish_audio.reference_id` | Fish Audio 音色 ID，永雏塔菲默认 `55b28b196e1c4fff9a55cd32a46eff25` |
| `fish_audio.model` | Fish Audio 模型，默认 `s2-pro` |
| `fish_audio.format` | Fish Audio 输出格式，默认 `mp3` |
| `fish_audio.latency` | Fish Audio 延迟模式，默认 `normal` |
| `fish_audio.speed` | Fish Audio 语速倍率，默认 `1.0` |
...
| `tts 你好哇` | 同上 |
| `永雏塔菲 你好哇` | 使用 Fish Audio 永雏塔菲音色合成语音 |
...
- 修改 Edge TTS / Fish Audio 的音色、API Key、默认引擎、语速、音量、音高、代理、长度限制、冷却时间和缓存目录。
...
HIKARI_BOT_NEO/
  ...
  Dockerfile                     # hikaribot 镜像构建
  docker-compose.yml             # 本地 Docker 全服务编排
  deploy/
    docker-compose.server.yml    # 服务器镜像部署编排
  docker/
