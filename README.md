...
- **Steam 解析正式接入**：`store.steampowered.com/app/<appid>` 商店游戏页现在能单独触发自动解析（此前只有同条消息里恰好带了别的受支持平台链接时才会被顺带解析），`parsers.steam` 显式声明输出模式，后台「权限规则」新增「Steam 解析」，媒体详情页平台清单同步；单个游戏页会产出预告片加截图等较多媒体，必要时调低 `max_send`
