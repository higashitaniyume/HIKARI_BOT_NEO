...
HIKARI BOT NEO 是一个基于 [NoneBot 2](https://nonebot.dev/) 的 QQ 机器人，通过 NapCat 的 OneBot V11 WebSocket 接入 QQ。它主要用于自动解析 QQ 消息里的媒体链接，并提供贴纸包、消息记录、错误通知等辅助能力。
...
| 消息记录 | 自动 | 私聊/群聊消息写入 `UserData` 下的 JSONL |
| 错误通知 | 自动 | 用户收到通用失败提示，管理员收到详细异常 |
...
- `BotData/config.json`、`BotData/plugin_configs/*.json`、`UserData`、日志和实际贴纸媒体文件默认不提交。
