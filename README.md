...
| 关于信息 | 私聊 `关于`；群聊 `@机器人 关于` | 查看机器人描述、版本、Git 提交、运行时长和贴纸库统计 |
| 错误通知 | 自动 | 用户收到通用失败提示，管理员收到脱敏后的异常 |

本仓库当前包含 Pixiv、抖音、Bilibili、TikTok、快手、微博、小红书、闲鱼、今日头条、小黑盒、Twitter/X、Instagram/Facebook、YouTube、媒体详情 Web、Telegram 贴纸、本地贴纸、本地语音、TTS、AI Agent、JMComic、osu!、Steam 热门热卖日报、AI 最新资讯日报、知乎热搜、定时推送框架、星露谷物语 Wiki 和 Minecraft Wiki 相关实现。聚合媒体解析能力来自 vendored 的 `drdon1234/astrbot_plugin_media_parser`。
...
部署和安装脚本会在启动前刷新根目录的 `version.json`，写入 `pyproject.toml` 中的版本号、当前 Git 提交 hash、短 hash、dirty 状态和生成时间。`关于` 命令优先读取这个文件；如果文件不存在，会回退到运行目录中的 `pyproject.toml` 和 Git 仓库信息。`version.json` 是部署生成物，不需要提交到 Git。
