...
| 贴纸上传页面 | 浏览器打开 `http://服务器IP:54213/` | 新建贴纸包，或上传贴纸到已有贴纸包 |
| JMComic PDF | 私聊 `jm <id>` | 下载并转换 PDF 后通过私聊发送，群聊不解析 |
| osu! 信息查询 | `osu` / `osu绑定` / `osu谱面` 等命令 | 查询用户、看板、成绩、排行榜、谱面，结果以图片发送 |
...
| `BotData/plugin_configs/sticker_web.json` | `password` |
...
所有 osu! 查询结果都会渲染为图片发送。QQ 绑定数据保存在 `UserData/osu_bindings.json`。
...
机器人启动后会用 Python 托管一个贴纸上传页面，默认监听：

```text
0.0.0.0:54213
```

> TCP 端口最大为 `65535`，因此不能使用 `542123`。如需修改端口，请改 `sticker_web.json` 中的 `port`。
