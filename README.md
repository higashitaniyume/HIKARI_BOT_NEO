...
本项目不再推荐使用 systemd 部署。根目录的 `docker-compose.yml` 会同时启动：

- `hikaribot`：本项目机器人
- `napcat`：QQ / OneBot 接入
- `cobalt`：Instagram / Facebook 媒体解析 API
- `astrbot`：其他平台解析机器人

### 本地启动

首次启动会根据 `.example.json` 生成本地配置文件：
...
脚本第一次生成配置后会退出。编辑 `BotData/config.json` 和 `BotData/plugin_configs/*.json`，再运行一次脚本即可启动。

### 部署到服务器

默认部署到 `root@192.168.31.2:/opt/hikaribot-dockcer`：
...
服务器上的运行数据集中放在 compose 所在目录，例如 `/opt/hikaribot-dockcer/BotData`、`/opt/hikaribot-dockcer/UserData`、`/opt/hikaribot-dockcer/napcat`、`/opt/hikaribot-dockcer/astrbot`。
