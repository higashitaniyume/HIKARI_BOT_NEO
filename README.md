...
| AI Agent 聊天 | 私聊直接发送文本；群聊 @机器人；也可用 `ai <内容>` | 调用兼容 OpenAI Chat Completions 的模型，并读取 BotData 中的女娲人格 skill |
...
开启后，私聊里发送文本会直接进入 AI Agent；群聊里必须 @机器人 才会回复。关闭后自动聊天不会响应，`ai <内容>` 命令会提示当前未启用。

关键字段：

| 字段 | 说明 |
|------|------|
| `enabled` | AI Agent 总开关，控制私聊自动回复、群聊 @ 回复和显式聊天命令 |
...
| 群聊 `@机器人 你好` | 使用当前模型和人格 skill 回复 |
| `ai 你好` / `aiagent 你好` / `聊天 你好` | 显式命令入口 |
| `重置` / `ai 重置` / `ai重置` | 清空当前私聊或群聊会话上下文 |
