...
def _enable_local_bilibili_interaction(
    parser: BaseVideoParser, cache_dir_available: bool
) -> None:
    ...
    if enable_rich_media and video_urls:
        ...
        for idx, url_list in enumerate(video_urls, 1):
            if url_list and isinstance(url_list, list) and len(url_list) > 0:
                ...
                backup_info = (
                    f" (备用URL: {backup_count}个)" if backup_count > 0 else ""
                )
                print(
                    f"  [{idx}] {main_url[:80]}{'...' if len(main_url) > 80 else ''}{backup_info}"
                )
...
async def prepare_bilibili_cookie_interaction(
    links_with_parser, session: aiohttp.ClientSession
) -> None:
    ...
    for _, parser in links_with_parser:
        ...
        auth_runtime = get_auth_runtime()
        if not (
            getattr(auth_runtime, "enabled", False)
            and getattr(auth_runtime, "local_debug_mode", False)
        ):
            continue
        ...
        timeout_seconds = max(
            1, int(getattr(parser, "admin_reply_timeout_minutes", 1440)) * 60
        )
        await run_bilibili_cookie_interaction_blocking(
            auth_runtime, session, timeout_seconds=timeout_seconds
        )


async def run_bilibili_cookie_interaction_blocking(
    auth_runtime, session: aiohttp.ClientSession, timeout_seconds: int
) -> str:
    ...
    try:
        ...
        qr.add_data(payload["login_url"])
        qr.make(fit=True)
        qr.print_ascii(invert=True)
    except Exception as e:
        logger.warning(f"[bilibili] 终端二维码生成失败，请直接打开登录链接: {e}")
    print("=" * 60)
    ...
    result = await auth_runtime.poll_login_until_complete(
        session, payload["qrcode_key"], timeout_seconds=max(1, timeout_seconds)
    )
...
async def parse_and_confirm_download(
    ...
    has_valid_media = enable_rich_media and any(
        ...
        and (bool(metadata.get("video_urls")) or bool(metadata.get("image_urls")))
        for metadata in metadata_list
    )
    ...
    while True:
        try:
            user_input = (
                input("输入 'y' 或 'yes' 下载，输入 'n' 或 'no' 跳过，输入 'q' 退出: ")
                .strip()
                .lower()
            )
...
async def main(
    ...
    if cache_dir_available:
        ...
        bilibili_cookie_runtime_file = os.path.join(bilibili_cookie_dir, "cookie.json")
    ...
    print(
        f"  B站Cookie文件: {bilibili_cookie_runtime_file or '不可用（缓存目录不可用）'}"
    )
    print("=" * 80)
